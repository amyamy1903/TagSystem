...
"""
删除一个租户下的客户标签
get:/api/tenant/:tenantId/platformType/:platformType/customer/tagId/:tagId/removeTag
"""
...
class DeleteCustomerTag(unittest.TestCase):
    ...
    def testDeleteCustomerTag(self):
        print("开始执行用例：--------------")
        time.sleep(0.5)
        headers = {
        ...
        url = host + '/api/tenant/' + self.tenant + '/platformType/' + self.platformtype + '/customer/tagId/' + self.tagid + '/removeTag'
        try:
        ...
        try:
            ...
            logging.error("测试数据是,tenant={tenant},platform_type={platform_type},tag_id={tag_id},code={code}".format
                          (tenant=self.tenant, platform_type=self.platformtype, tag_id=self.tagid, code=self.code))
